...
class SwaggerAdditionsGenerator {
  ...
  String generateIndexes(List<String> files) {
    final importsList = files.map((String fileName) {
      final sanitizedName = fileName.replaceAll('-', '_');
      final className = getClassNameFromFileName(sanitizedName);

      return 'export \'$sanitizedName.dart\' show $className;';
    }).toList();
  ...
  String generateConverterMappings(List<String> files, bool hasModels) {
    ...
    files.forEach((String fileName) {
      final className =
          '${getClassNameFromFileName(fileName)}$converterClassEnding';
